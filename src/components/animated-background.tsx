...
/**
 * Safely set a Spline variable — the .spline editor format does not export
 * named variables (only .splinecode does), so getVariable returns undefined
 * and setVariable logs a console warning. This helper silences that noise
 * without changing any behaviour.
 */
const safeSetVar = (app: Application, name: string, value: string) => {
  try {
    if (app.getVariable(name) !== undefined) {
      app.setVariable(name, value);
    }
  } catch {
    // variable doesn't exist in this scene — ignore
  }
};
...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      ...
      if (splineApp.getVariable("heading") && splineApp.getVariable("desc")) {
        safeSetVar(splineApp, "heading", "");
        safeSetVar(splineApp, "desc", "");
      }
    } else {
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyDown", (e) => {
      ...
      if (skill) {
        ...
        setSelectedSkill(skill);
        selectedSkillRef.current = skill;
        safeSetVar(splineApp, "heading", skill.label);
        safeSetVar(splineApp, "desc", skill.shortDescription);
      }
    });
  ...
  useEffect(() => {
    ...
    safeSetVar(splineApp, "heading", selectedSkill.label);
    safeSetVar(splineApp, "desc", selectedSkill.shortDescription);
  }, [selectedSkill]);
